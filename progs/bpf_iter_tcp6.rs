...
// Direct translation of tools/testing/selftests/bpf/progs/bpf_iter_tcp6.c
// (bpf-rs-core idiom), reusing every technique verified for
// progs/bpf_iter_tcp4.rs (see that file's header + memory note
// [[bpf_iter_tcp4]] for the full rationale). Highlights specific to the v6
// variant:
//
// `struct tcp6_sock { struct tcp_sock tcp; struct ipv6_pinfo inet6; };` --
// `tcp` is the first member, so a `*mut tcp_sock` obtained by reinterpreting
// `bpf_skc_to_tcp6_sock`'s return is byte-identical to `&tp->tcp`, and the
// whole offset-0 chain tcp4 already relies on (`tcp_sock` -> `inet_conn`
// (`inet_connection_sock`) -> `icsk_inet` (`inet_sock`) -> `sk` (`sock`) ->
// `__sk_common` (`sock_common`)) applies unchanged one level deeper. Per the
// same CO-RE reasoning tcp4 documents, `btf_struct_walk` validates purely by
// (root_type, byte_offset) against the helper's real fixed return type
// (`tcp6_sock`), independent of our local type's name -- offsets computed
// against local `tcp_sock` land on the identical bytes since the embedding
// is at offset 0.
//
// `struct sock_common` additionally carries the real (non-macro) fields
// `skc_v6_daddr`/`skc_v6_rcv_saddr` (`struct in6_addr`) used here via the
// `sk_v6_daddr`/`tw_v6_daddr`/`ir_v6_*` macros in bpf_tracing_net.h, all of
// which just spell out `__sk_common`/`__tw_common`/`__req_common` field
// access -- so `dump_tw_sock`/`dump_req_sock` reach v6 addresses the same
// way tcp4 reaches v4 ones: casting the timewait/request pointer directly to
// `*mut sock_common` (offset-0 embedding, unchanged from tcp4).
//
// `struct in6_addr { union { u8 u6_addr8[16]; ... u32 u6_addr32[4]; }
// in6_u; }`: `in6_u` is a *named* union member (must be declared and
// traversed explicitly, see [[btf-named-union-member-no-auto-flatten]],
// already used in bpf_iter_ipv6_route.rs). Declaring only the `u6_addr32`
// member (matching `s6_addr32`'s macro target) is enough -- CO-RE resolves
// a union member purely by name at the target offset (0 for every member
// here), so the other union members don't need to exist locally.
//
// All non-address logic (jiffies helpers, the `icsk_ack.ato` /
// `request_sock.num_timeout` bitfield-sibling-byte workaround, the
// branch-merge-safe `#[inline(never)]` readers, `sock_i_ino`, `CONFIG_HZ`
// hardcoded to 1000) is copied verbatim from bpf_iter_tcp4.rs -- none of it
// is v4-specific.

use bpf_rs_core::bpf_object;
use bpf_rs_core::helpers::{
    bpf_jiffies64, bpf_probe_read_kernel, bpf_seq_printf, bpf_skc_to_tcp6_sock,
    bpf_skc_to_tcp_request_sock, bpf_skc_to_tcp_timewait_sock,
};
use btf_macros::btf;
use core::ffi::c_void;
...
fn dump_tcp6_sock(seq: *mut c_void, tp: *mut tcp_sock, uid: u32, seq_num: u32) -> i32 {
    ...
    let dest_ptr = skc_ref.skc_v6_daddr().in6_u().u6_addr32().as_ptr() as *const u32;
    let src_ptr = skc_ref.skc_v6_rcv_saddr().in6_u().u6_addr32().as_ptr() as *const u32;
    ...
    static FMT_ADDR: [u8; 50] = *b"%4d: %08X%08X%08X%08X:%04X %08X%08X%08X%08X:%04X \0";
    let params_addr: [u64; 11] = [
        seq_num as u64,
        src0 as u64,
        src1 as u64,
        src2 as u64,
...
fn dump_req_sock(seq: *mut c_void, req: *mut request_sock, uid: u32, seq_num: u32) -> i32 {
    ...
    // C names these `src`/`dest` from `ir_v6_loc_addr`/`ir_v6_rmt_addr`.
    let src_ptr = skc_ref.skc_v6_rcv_saddr().in6_u().u6_addr32().as_ptr() as *const u32;
    let dest_ptr = skc_ref.skc_v6_daddr().in6_u().u6_addr32().as_ptr() as *const u32;
    let (src0, src1, src2, src3) = unsafe {
        (
    ...
    // `num_retrans` byte (see bpf_iter_tcp4.rs for why `num_timeout` isn't
    // read as a #[btf] field directly).
...
#[no_mangle]
extern "C" fn dump_tcp6(ctx: *const bpf_iter__tcp) -> i32 {
    let ctx = unsafe { &*ctx };
    ...
    if seq_num == 0 {
        static FMT0: [u8; 146] = *b"  sl  local_address                         remote_address                        st tx_queue rx_queue tr tm->when retrnsmt   uid  timeout inode\n\0";
        bpf_seq_printf(
            meta.seq,
    ...
    let tp = bpf_skc_to_tcp6_sock(sk_common as *mut c_void) as *mut tcp_sock;
    if !tp.is_null() {
        return dump_tcp6_sock(meta.seq, tp, uid, seq_num);
    }

    let tw = bpf_skc_to_tcp_timewait_sock(sk_common as *mut c_void) as *mut inet_timewait_sock;
    if !tw.is_null() {
        return dump_tw_sock(meta.seq, tw, uid, seq_num);
    }

    let req = bpf_skc_to_tcp_request_sock(sk_common as *mut c_void) as *mut request_sock;
    if !req.is_null() {
