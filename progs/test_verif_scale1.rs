...
// Direct translation of tools/testing/selftests/bpf/progs/test_verif_scale1.c
// (bpf-rs-core idiom), inlining test_jhash.h's jhash().
//
// The C source repeats a "compute jhash(ptr, 14, cb[0]+i); i++" step 90
// times via macro expansion (`C30;C30;C30;` — a scale/verifier-stress test,
// not a real loop: each unrolled copy re-does the bounds check, and once
// `ptr + nh_off > data_end` a copy's do-while breaks without touching `i`,
// so every remaining copy re-fails the same check and no-ops through to
// the end of the function). A bounded `for` loop with `break` reproduces
// that behavior exactly while keeping the translation load-bearing rather
// than 90x duplicated.
...
extern "C" fn balancer_ingress(ctx: *mut __sk_buff) -> i32 {
    ...
    for _ in 0..90u32 {
        ...
        let hash = jhash(ptr as *const u8, NH_OFF as u32, cb0.wrapping_add(i));
        vstore!((*ctx).tc_index, hash);
        i += 1;
    }
