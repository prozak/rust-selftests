...
// bpf-rs-core idiom.
//
// ringbuf has no key/value/max_entries in the C source (userspace sets
// max_entries at runtime via skel->maps.ringbuf.max_entries before load),
// so its BTF map struct carries only the type member — bpf_map! escape
// hatch.
...
extern "C" fn test_ringbuf(_ctx: *const c_void) -> i32 {
    ...
    let cur_flags = unsafe { flags } as u64;
    let sample =
        bpf_ringbuf_reserve(&ringbuf, core::mem::size_of::<Sample>() as u64, 0) as *mut Sample;
    if sample.is_null() {
    ...
    if unsafe { (*sample).seq } & 1 != 0 {
        ...
        bpf_ringbuf_discard(sample as *mut c_void, cur_flags);
        sync_fetch_and_add(core::ptr::addr_of_mut!(discarded), 1);
    } else {
        bpf_ringbuf_submit(sample as *mut c_void, cur_flags);
    }
