...
// tools/testing/selftests/bpf/progs/test_l4lb_noinline.c (bpf-rs-core
// idiom). Functionally identical to test_l4lb.c save for get_packet_dst's
// extra hash-validity guard (checked before the %RING_SIZE reduction) and
// __noinline annotations on the C side, which don't affect this
// translation.

use core::ffi::c_void;

use bpf_rs_core::ctx::{__sk_buff, TC_ACT_OK, TC_ACT_SHOT};
use bpf_rs_core::helpers::{bpf_map_lookup_elem, bpf_redirect, bpf_skb_set_tunnel_key};
...
/// algorithm's one 12-byte mix round plus a length==4 tail (`a += k[3]`,
/// a raw little-endian word add) collapses to exactly this for length 16.
#[inline(always)]
...
// struct ctl_value (test_l4lb_noinline.c): stack/map-value scratch, only
// ever read via `.ifindex`.
#[repr(C)]
union CtlValue {
    #[allow(dead_code)]
    value: u64,
...
// test_l4lb_noinline.c's get_packet_dst additionally rejects any hash that
// isn't one of the two literal jhash outputs the test traffic produces,
// before reducing mod RING_SIZE (test_l4lb.c instead reduces first and has
// no such guard).
#[inline(always)]
fn get_packet_dst(
    ...
    let hash = get_packet_hash(pckt, is_ipv6);
    let key = RING_SIZE
...
#[inline(always)]
fn parse_udp(data: usize, off: u64, data_end: usize, pckt: &mut PacketDescription) -> bool {
...
#[inline(always)]
fn parse_tcp(data: usize, off: u64, data_end: usize, pckt: &mut PacketDescription) -> bool {
...
fn process_packet(
    ...
    if is_ipv6 {
        let ip6h = (data + off as usize) as *const Ipv6Hdr;
        if ip6h as usize + core::mem::size_of::<Ipv6Hdr>() > data_end {
            return TC_ACT_SHOT;
        }
        ...
        if nexthdr == IPPROTO_FRAGMENT {
            ...
            let action = parse_icmpv6(data, data_end, off, &mut pckt);
            if action >= 0 {
                return action;
            }
        ...
        let iph = (data + off as usize) as *const IpHdr;
        if iph as usize + core::mem::size_of::<IpHdr>() > data_end {
            return TC_ACT_SHOT;
        }
        if unsafe { (*iph).version_ihl } & 0x0f != 5 {
        ...
        if protocol == IPPROTO_ICMP {
            let action = parse_icmp(data, data_end, off, &mut pckt);
            if action >= 0 {
                return action;
            }
    ...
    unsafe { core::ptr::write_unaligned(pkt_start as *mut u32, remote_ipv4) };
