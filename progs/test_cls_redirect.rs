...
// (plain build, SUBPROGS undefined), bpf-rs-core idiom. Sibling of
// test_cls_redirect_subprogs.rs, which translates the same C source built
// with -DSUBPROGS; the two files share almost all of their logic and differ
// only in which helper functions stay as real subprograms.
//
...
//
// - Without SUBPROGS, the C source's `INLINING` macro is `__always_inline`
//   (not `__noinline`), so every `static INLINING` helper collapses into the
//   single `cls_redirect` verifier-visible function — the C object exports
//   only `cls_redirect`, `metrics_map`, `ENCAPSULATION_PORT/IP` and the
//   license as globals, no bpf2bpf subprogram symbols. Every such helper here
//   is a plain private `#[inline(always)] fn`, matching. Only the functions
//   the C source marks `__always_inline` unconditionally (buf_* and the two
//   pkt_parse_* that return pointers into the caller's own stack scratch)
//   were already inline in the SUBPROGS sibling — pkt_parse_ipv6 in
//   particular must actually inline, since a real subprogram cannot return a
//   pointer into its own stack frame.
...
// core::ptr::copy_nonoverlapping (and not even a plain
// read_unaligned/write_unaligned loop, which LLVM's MemCpyOpt pass still
// recognizes and rewrites back into an llvm.memcpy call): with every
// INLINING helper folded into one function (this file's whole point, vs. the
// SUBPROGS sibling), several small fixed-size copies (the 16-byte in6_addr
// copies in fill_tuple/process_icmpv6, the 6-byte ETH_ALEN swap in
// forward_to_next_hop) end up duplicated across inlined call sites and
// become recognizable memcpy-shaped code that add_ksyms.py turns into an
// extern bpf_arena_memcpy kfunc call — not in this kernel's BTF outside
// arena progs, and its R_BPF_64_ABS32 relocation entry also risks the
// btf_rename.py .rel.BTF sh_info corruption bug (see
...
#[inline(always)]
fn pkt_skip_ipv4_options(buf: *mut Buf, ipv4: *const IpHdr) -> bool {
...
#[inline(always)]
fn pkt_parse_icmp_l4_ports(pkt: *mut Buf, ports: *mut FlowPorts) -> bool {
...
#[inline(always)]
fn skip_next_hops(pkt: *mut Buf, n: u8) -> i32 {
    match n {
        1 => {
...
#[inline(always)]
fn get_next_hop(pkt: *mut Buf, encap: *const EncapHeaders, next_hop: *mut InAddr) -> i32 {
    let next_hop_idx = pget!((*encap).unigue.next_hop);
    let hop_count = pget!((*encap).unigue.hop_count);
...
#[inline(always)]
fn process_icmpv4(pkt: *mut Buf, metrics: *mut Metrics) -> i32 {
...
#[inline(always)]
fn process_icmpv6(pkt: *mut Buf, metrics: *mut Metrics) -> i32 {
...
#[inline(always)]
fn process_tcp(pkt: *mut Buf, iph: *const c_void, iphlen: u64, metrics: *mut Metrics) -> i32 {
...
#[inline(always)]
fn process_udp(pkt: *mut Buf, iph: *const c_void, iphlen: u64, metrics: *mut Metrics) -> i32 {
...
#[inline(always)]
fn process_ipv4(pkt: *mut Buf, metrics: *mut Metrics) -> i32 {
...
#[inline(always)]
fn process_ipv6(pkt: *mut Buf, metrics: *mut Metrics) -> i32 {
