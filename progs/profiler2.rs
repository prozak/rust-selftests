...
// Direct translation of tools/testing/selftests/bpf/progs/profiler2.c
// (== profiler.inc.h with UNROLL undefined and INLINE empty/plain
// `static`), bpf-rs-core idiom.
//
// profiler.inc.h is shared verbatim by profiler{1,2,3}.c; UNROLL and
// INLINE are pure codegen hints (pragma-unroll loops vs plain loops,
// force-inline vs compiler-decides/never-inline) with zero effect on
// observable behavior -- the computed field values, control flow and
// return values are identical to profiler1. prog_tests/test_profiler.c
// treats profiler2 exactly like profiler1: open_and_load() + attach()
// (plus the pristine profiler1/profiler3 clang objects) and then
...
// `struct linux_binprm *`. See profiler1.rs for the full field-by-field
// rationale (CO-RE hops, cgroup-v1 omission, version-compat hardcoding);
// this file mirrors it 1:1 since the underlying C source is identical
// modulo those inlining/unrolling hints.
//
...
//
// profiler2.c's INLINE is empty (plain `static`, compiler decides) rather
// than __always_inline, but that's a codegen hint only -- clang's own
// inliner is free to inline these single-call-site static helpers anyway.
// `#[inline(always)]` is kept on every helper below (same as profiler1.rs):
// it's a valid rendering of "compiler-decides" for this call graph, keeps
// each SEC program to one flat stack frame, and the BPF verifier's 512-byte
// stack limit is cumulative across non-inlined call frames -- none of these
// helpers' locals are more than a handful of pointers/integers, so full
// inlining keeps every program comfortably under the limit (all the
// genuinely large state -- var_*_data_t payloads -- lives in map memory,
// never on stack).
