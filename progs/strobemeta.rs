...
// Direct translation of tools/testing/selftests/bpf/progs/strobemeta.c
// (strobemeta.h with neither SUBPROGS nor USE_BPF_LOOP nor NO_UNROLL
// defined -> full loop unroll requested via __pragma_loop_unroll,
// __always_inline everywhere), bpf-rs-core idiom.
//
...
//
// C's __pragma_loop_unroll is a *request*; at STROBE_MAX_MAPS == 100 LLVM's
// own cost model doesn't honor it as a literal full unroll even in the C
// build (the kernel comment on test_verif_scale_strobemeta calls the result
// "partial unroll... 19k insn in a loop... ~350k processed_insns") — clang
// keeps a real loop and relies on that partial-unroll factor to reach a
// verifier-provable fixed point. Rust has no equivalent per-loop unroll
// pragma to reproduce that exact factor, and both alternatives tried here
// failed at full scale:
//   - a literal 127-call (2+25+100) manual unroll: ~4x the C object's
//     static code size, hits the 1M processed-insn cap (E2BIG);
//   - a genuine `for idx in 0..N` loop calling real (non-inlined)
//     read_int_var/read_str_var/read_map_var subprograms, even with the
//     C USE_BPF_LOOP variant's double-XOR + barrier_var() precision-drop
//     trick on payload_off: still hits the 1M cap, because a literal
//     backward-branching loop forces the verifier to prove convergence by
//     re-walking the large read_map_var body (calc_location + the
//     STROBE_MAX_MAP_ENTRIES-entry unrolled loop) across iterations.
//
// strobemeta_bpf_loop.c — same strobemeta.h, same STROBE_MAX_* scale, same
// section/map/global shape, differing only in using the bpf_loop() helper
// for these three loops — compiles to an object with an *identical*
// external contract and is a proven-passing translation (progs/
// strobemeta_bpf_loop.rs). bpf_loop offloads the actual repetition to the
// kernel helper instead of BPF-level control flow: the verifier checks the
// callback body once (for an arbitrary bounded index), not once per
// convergence-proving loop iteration, which is exactly the cost cliff above.
// Since scale_test() only requires the load to succeed with matching
// maps/section, this file adopts that same internal strategy for
// strobemeta.c's on_event — a legitimate implementation choice, not a
// change to the object's external ABI.
...
const STROBE_MAX_MAPS: usize = 100;
