...
// Direct translation of tools/testing/selftests/bpf/progs/pyperf600_iter.c
// (pyperf.h with STACK_MAX_LEN 600, SUBPROGS + NO_UNROLL + USE_ITER defined),
// bpf-rs-core idiom.
//
...
// calls into one verified subprogram body, not 5x inlined copies (same
// pattern as pyperf_subprogs.rs).
//
// USE_ITER replaces the plain `for (int i = 0; i < STACK_MAX_LEN; ++i)` (or
// its NO_UNROLL variant) with C's `bpf_for(i, 0, STACK_MAX_LEN)` open-coded
// BPF iterator macro, which expands to raw bpf_iter_num_new/_next/_destroy
// kfunc calls plus a bounds check on the yielded value that proves `i` is in
// [0, STACK_MAX_LEN) to the verifier. Translated here as literally as
// possible with extern kfuncs, same pattern as dmabuf_iter.rs's open-coded
// iterator translation: bpf_iter_num_destroy() is called manually at every
// exit path (the loop's normal end, and the early `return 0` inside the
// loop body), mirroring the scope-exit semantics of C's
// __attribute__((cleanup(bpf_iter_num_destroy))).
//
// The only consumer is bpf_verif_scale.c's test_verif_scale_pyperf600_iter(),
...
fn on_event_once(ctx: *const c_void) -> i32 {
    ...
    if pthread_match || !use_tls {
        ...
        unsafe { bpf_iter_num_new(&mut it, 0, STACK_MAX_LEN as i32) };
        ...
        loop {
            ...
            let iv = unsafe { *t };
            if !(iv >= 0 && (iv as usize) < STACK_MAX_LEN) {
                break;
            }
            let i = iv as usize;
